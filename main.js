...
var categories = {
  ...
  'sports & outdoors': {36: "organized sport", 37: "outdoor recreation", 38: "fitness", 10136: "races"},
...
function postEvent(title, primCategory, primSubCategory, secCategory, secSubCategory, locationName, address, city, state, description, date, startTime, endTime, timeValue, url, host, contactNumber, lat, lng) {
  new Event({
    title: title,
  ...
  catArr.forEach(function(eventCatNum) {
    ...
    for(var cat in categories) {
      ...
      for(var subCatNum in subCategories) {
        ...
        if(subCatNum == eventCatNum) {
          console.log(`category: ${cat}`)
          console.log(`subcategory: ${subCategories[subCatNum]}`);
        } else {
...
fs.readFile(__dirname + '/getevents.xml', function(err, data) {
  xml2js.parseString(data, {tagNameProcessors: [processors.stripPrefix], explicitArray: false}, function (err, result) {
    ...
    var eventsArr = events.map(function(event){
      ...
      var primCategory = "music";
      var primSubCategory = "rock";
      var secCategory = "art";
      var secSubCategory = "art show";
      ...
      //       postEvent(title, primCategory, primSubCategory, secCategory, secSubCategory, locationName, address, city, state, description, date, startTime, endTime, timeValue, url, host, contactNumber, lat, lng);
      //     });
      // } else {
